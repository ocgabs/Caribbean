MODULE dtatsd
   ...
   LOGICAL , PUBLIC ::   ln_tsd_interp    !: vertical interpolation flag !! VAL LINE ADDED
   ...
   INTEGER                                 ::   jpk_init , inum_dta                !! VAL LINE ADDED
   INTEGER                                 ::   id ,linum   ! local integers       !! VAL LINE ADDED
   INTEGER                                 ::   zdim(4)                            !! VAL LINE ADDED
...
CONTAINS
   ...
   SUBROUTINE dta_tsd_init( ld_tradmp )
      ...
      !
      ! VAL LINE COMMENTED : INTEGER ::   ios, ierr0, ierr1, ierr2, ierr3   ! local integers
      INTEGER ::   ios, ierr0, ierr1, ierr2, ierr3, ierr4, ierr5   ! local integers  !! VAL LINE ADDED
      !!
      CHARACTER(len=100)            ::   cn_dir          ! Root directory for location of ssr files
      ! VAL LINE COMMENTED  : TYPE(FLD_N), DIMENSION( jpts) ::   slf_i           ! array of namelist informations on the fields to read
      ! VAL LINE COMMENTED  : TYPE(FLD_N)                   ::   sn_tem, sn_sal
      TYPE(FLD_N), DIMENSION(jpts+2)::   slf_i           ! array of namelist informations on the fields to read ! VAL LINE ADDED
      TYPE(FLD_N)                   ::   sn_tem, sn_sal, sn_dep, sn_msk                                         ! VAL LINE ADDED

      !!
      ! VAL LINE COMMENTED : NAMELIST/namtsd/   ln_tsd_init, ln_tsd_dmp, cn_dir, sn_tem, sn_sal
      NAMELIST/namtsd/   ln_tsd_init, ln_tsd_interp, ln_tsd_dmp, cn_dir, sn_tem, sn_sal, sn_dep, sn_msk      ! VAL LINE ADDED
      !!----------------------------------------------------------------------
      !
      !  Initialisation
      ! VAL LINE COMMENTED : ierr0 = 0  ;  ierr1 = 0  ;  ierr2 = 0  ;  ierr3 = 0
      ierr0 = 0  ;  ierr1 = 0  ;  ierr2 = 0  ;  ierr3 = 0  ; ierr4 = 0  ;  ierr5 = 0                            !VAL LINE ADDED
      !
...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'namtsd in configuration namelist' )
      ...
      IF(lwp) THEN                  ! control print
         ...
         WRITE(numout,*) '      iInterpolation of initial conditions in the vertical ln_tsd_interp = ', ln_tsd_interp         ! VAL LINE ADDED
      ...
      IF( ln_tsd_init .OR. ln_tsd_dmp ) THEN
         !
         ! VAL LINE COMMENTED : ALLOCATE( sf_tsd(jpts), STAT=ierr0 )
         ! VAL START OF ADDED LINES
         ...
         ENDIF
         !
         ! VAL LINE COMMETED  :                        ALLOCATE( sf_tsd(jp_tem)%fnow(jpi,jpj,jpk)   , STAT=ierr0 )
         ! VAL LINE COMMENTED : IF( sn_tem%ln_tint )   ALLOCATE( sf_tsd(jp_tem)%fdta(jpi,jpj,jpk,2) , STAT=ierr1 )
         ! VAL LINE COMMENTED :                       ALLOCATE( sf_tsd(jp_sal)%fnow(jpi,jpj,jpk)   , STAT=ierr2 )
         ! VAL LINE COMMENTED : IF( sn_sal%ln_tint )   ALLOCATE( sf_tsd(jp_sal)%fdta(jpi,jpj,jpk,2) , STAT=ierr3 )
         !
         ! VAL START ADDED LINES
         IF( ln_tsd_interp ) THEN
         ...
         ! VAL END OF ADDED LINES
         !
         ! VAL COMMENTED LINE : IF( ierr0 + ierr1 + ierr2 + ierr3 > 0 ) THEN
         IF( ierr0 + ierr1 + ierr2 + ierr3 + ierr4 + ierr5 > 0 ) THEN  ! VAL ADDED LINE
         ...
         IF( ln_tsd_interp ) slf_i(jp_dep) = sn_dep   ;   slf_i(jp_msk) = sn_msk         ! VAL ADDED LINES
   ...
   SUBROUTINE dta_tsd( kt, ptsd )
      ...
      !
      ! VAL COMMENTED LINE : INTEGER ::   ji, jj, jk, jl, jkk   ! dummy loop indicies
      ! VAL COMMENTED LINE : INTEGER ::   ik, il0, il1, ii0, ii1, ij0, ij1   ! local integers
      INTEGER ::   ji, jj, jk, jl, jk_init   ! dummy loop indicies          ! VAL ADDED LINE
      INTEGER ::   ik, il0, il1, ii0, ii1, ij0, ij1        ! local integers ! VAL ADDED LINE

      REAL(wp)::   zl, zi                             ! local scalars
      ! VAL COMMENTED LINE : REAL(wp), DIMENSION(jpk) ::  ztp, zsp   ! 1D workspace
      !!----------------------------------------------------------------------
      !
...
!!gm end
      !
      ! VAL COMMENTED LINE :    ptsd(:,:,:,jp_tem) = sf_tsd(jp_tem)%fnow(:,:,:)    ! NO mask
      ! VAL COMMENTED LINE :    ptsd(:,:,:,jp_sal) = sf_tsd(jp_sal)%fnow(:,:,:) 
      ! VAL START OF ADDED LINES
      ...
      ! VAL END OF ADDED LINES
      !
      ! VAL COMMENTED LINE :  IF( ln_sco ) THEN                   !==   s- or mixed s-zps-coordinate   ==!
      ! VAL COMMENTED LINE :    !
      ! VAL COMMENTED LINE :    IF( kt == nit000 .AND. lwp )THEN
      ! VAL COMMENTED LINE :       WRITE(numout,*)
      ! VAL COMMENTED LINE :       WRITE(numout,*) 'dta_tsd: interpolates T & S data onto the s- or mixed s-z-coordinate mesh'
      ! VAL COMMENTED LINE :    ENDIF
      ! VAL COMMENTED LINE :    !
      ! VAL COMMENTED LINE :    DO jj = 1, jpj                         ! vertical interpolation of T & S
      ! VAL COMMENTED LINE :       DO ji = 1, jpi
      ! VAL COMMENTED LINE :          DO jk = 1, jpk                        ! determines the intepolated T-S profiles at each (i,j) points
      IF( ln_tsd_interp ) THEN                 ! probably should use pointers in the following to make more readable  ! VAL ADDED LINE
      !
         DO jk = 1, jpk                        ! determines the intepolated T-S profiles at each (i,j) points         ! VAL ADDED LINE
            DO jj= 1, jpj                                                                                             ! VAL ADDED LINE
               DO ji= 1, jpi                                                                                          ! VAL ADDED LINE
                  zl = gdept_0(ji,jj,jk)
                  ! VAL LINE COMMENTED :  IF(     zl < gdept_1d(1  ) ) THEN          ! above the first level of data
                  ! VAL LINE COMMENTED :   ztp(jk) =  ptsd(ji,jj,1    ,jp_tem)
                  ! VAL LINE COMMENTED :   zsp(jk) =  ptsd(ji,jj,1    ,jp_sal)
                  ! VAL LINE COMMENTED : ELSEIF( zl > gdept_1d(jpk) ) THEN          ! below the last level of data
                  ! VAL LINE COMMENTED :   ztp(jk) =  ptsd(ji,jj,jpkm1,jp_tem)
                  ! VAL LINE COMMENTED :   zsp(jk) =  ptsd(ji,jj,jpkm1,jp_sal)
                  ! VAL LINE COMMENTED :ELSE                                      ! inbetween : vertical interpolation between jkk & jkk+1
                  ! VAL LINE COMMENTED :   DO jkk = 1, jpkm1                                  ! when  gdept(jkk) < zl < gdept(jkk+1)
                  ! VAL LINE COMMENTED :      IF( (zl-gdept_1d(jkk)) * (zl-gdept_1d(jkk+1)) <= 0._wp ) THEN
                  ! VAL LINE COMMENTED :         zi = ( zl - gdept_1d(jkk) ) / (gdept_1d(jkk+1)-gdept_1d(jkk))
                  ! VAL LINE COMMENTED :         ztp(jk) = ptsd(ji,jj,jkk,jp_tem) + ( ptsd(ji,jj,jkk+1,jp_tem) - ptsd(ji,jj,jkk,jp_tem) ) * zi 
                  ! VAL LINE COMMENTED :         zsp(jk) = ptsd(ji,jj,jkk,jp_sal) + ( ptsd(ji,jj,jkk+1,jp_sal) - ptsd(ji,jj,jkk,jp_sal) ) * zi
                  ! VAL START OF ADDED LINE :
                  ...
                  ! VAL END OF ADDED LINE
                        ENDIF
                     END DO
                  ENDIF
               ENDDO    ! VAL LINE ADDED
            ENDDO       ! VAL LINE ADDED
               ! VAL LINE COMMENTED : END DO
               ! VAL LINE COMMENTED : DO jk = 1, jpkm1
               ! VAL LINE COMMENTED :   ptsd(ji,jj,jk,jp_tem) = ztp(jk) * tmask(ji,jj,jk)     ! mask required for mixed zps-s-coord
               ! VAL LINE COMMENTED :   ptsd(ji,jj,jk,jp_sal) = zsp(jk) * tmask(ji,jj,jk)
               ! VAL LINE COMMENTED :END DO
               ! VAL LINE COMMENTED :ptsd(ji,jj,jpk,jp_tem) = 0._wp
               ! VAL LINE COMMENTED :ptsd(ji,jj,jpk,jp_sal) = 0._wp
            ! VAL LINE COMMENTED : END DO
         END DO
         !
         ptsd(:,:,:,jp_tem) = ptsd(:,:,:,jp_tem) *tmask(:,:,:) ! VAL LINE ADDED
         ptsd(:,:,:,jp_sal) = ptsd(:,:,:,jp_sal) *tmask(:,:,:) ! VAL LINE ADDED
      ELSE                                !==   z- or zps- coordinate   ==!
         !                             
         ! VAL LINE COMMENTED :  ptsd(:,:,:,jp_tem) = ptsd(:,:,:,jp_tem) * tmask(:,:,:)    ! Mask
         ! VAL LINE COMMENTED :  ptsd(:,:,:,jp_sal) = ptsd(:,:,:,jp_sal) * tmask(:,:,:)
         ptsd(:,:,:,jp_tem) = sf_tsd(jp_tem)%fnow(:,:,:)  * tmask(:,:,:)  ! Mask                         ! VAL LINE ADDED
         ptsd(:,:,:,jp_sal) = sf_tsd(jp_sal)%fnow(:,:,:)  * tmask(:,:,:)                                 ! VAL LINE ADDED
         !
      ...
      IF( .NOT.ln_tsd_dmp ) THEN                   !==   deallocate T & S structure   ==! 
         ...
         IF( ln_tsd_interp )            DEALLOCATE( sf_tsd(jp_dep)%fnow )     ! T arrays in the structure        ! VAL LINE ADDED
         IF( ln_tsd_interp )            DEALLOCATE( sf_tsd(jp_msk)%fnow )     ! T arrays in the structure        ! VAL LINE ADDED
