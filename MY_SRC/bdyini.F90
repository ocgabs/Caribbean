...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'nambdy in configuration namelist' )
   ...
   SUBROUTINE bdy_def
      ...
      ! -----------------------------------------
!(VAL COMMENT THIS LINE)      IF( jperio /= 0 )   CALL ctl_stop( 'bdy_segs: Cyclic or symmetric,',   &
!(VAL COMMENT THIS LINE)         &                               ' and general open boundary condition are not compatible' )
